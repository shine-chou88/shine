	...
	<div class="Section0"
						...
						<div align=center>
							<table class=MsoNormalTable border=1 cellspacing=0
									...
									<td width=523 valign=top
										...
										<p class=MsoNormal
											...
											<span
												...
												普通护照（<c:if test="${fn:contains(bean.applyHandleType,'privatePassport')}">√</c:if>）
												港澳通行证（<c:if test="${fn:contains(bean.applyHandleType,'privateHKAndMacaoPass')}">√</c:if>）
												台湾通行证（<c:if test="${fn:contains(bean.applyHandleType,'privateTaiwanPass')}">√</c:if>）
											</span><span
												style="font-family: 仿宋_GB2312; font-size: 15.0000pt; mso-font-kerning: 1.0000pt;"><o:p></o:p></span>
										</p>
										<p class=MsoNormal
										...
										<p class=MsoNormal
											...
											<span
												style="mso-spacerun: 'yes'; font-family: 仿宋_GB2312; font-size: 15.0000pt; mso-font-kerning: 1.0000pt;">
												普通护照（<c:if test="${fn:contains(bean.applyUseType,'privatePassport')}">√</c:if>）
												港澳通行证（<c:if test="${fn:contains(bean.applyUseType,'privateHKAndMacaoPass')}">√</c:if>）
												台湾通行证（<c:if test="${fn:contains(bean.applyUseType,'privateTaiwanPass')}">√</c:if>）
											</span><span
												style="font-family: 仿宋_GB2312; font-size: 15.0000pt; mso-font-kerning: 1.0000pt;"><o:p></o:p></span>
										</p>
										<p class=MsoNormal
								...
								<!-- 记录 -->
								<c:choose>
									<c:when test="${currentUser.hasRole('ORGANIZATION_STAFF')==true &&taskDefinitionKey=='ORGANIZATION_STAFF'}">
